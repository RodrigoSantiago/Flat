...
public class Area implements Shape, Cloneable {
    ...
    public Area(Shape s) {
        this(s.pathIterator(null));
    }

    public Area(PathIterator pi) {
    ...
    public boolean contains(float x, float y, float width, float height) {
        int crossCount = Crossing.intersectPath(pathIterator(null), x, y,
                width, height);
    ...
    public void transform(Affine t) {
        copy(new Area(pathIterator(t)), this);
    }

    public Area createTransformedArea(Affine t) {
        return new Area(pathIterator(t));
    }
    ...
    private class AreaPathIterator implements PathIterator {
        ...
        AreaPathIterator(Area area, Affine t) {
            this.area = area;
            this.transform = t;
        }
        ...
        public int currentSegment(double[] c) {
            ...
            if (transform != null) {
                ...
                for (int i = 0; i < count; i++) {
                    double x = c[srcOff++], y = c[srcOff++];
                    c[dstOff++] = transform.m00 * x + transform.m10 * y + transform.m02;
                    c[dstOff++] = transform.m01 * x + transform.m11 * y + transform.m12;
                }
            }
            for (int i = 0; i < c.length; i++) {
                if (Double.isNaN(c[i])) c[i] = 0;
            }
