...
/**
 * A 4x4 matrix; useful for 3D transforms.
 */
public class Matrix4 implements Serializable {
     ...
     *
     * @param values The float array to read
     */
    public Matrix4(float[] values, int offset) {
     ...
     * @return This matrix for chaining
     */
    public Matrix4 set(float[] values, int offset) {
     ...
     *
     * @return This matrix for chaining
     * @throws NoninvertibleTransformException if the matrix is singular (not invertible)
     */
    public Matrix4 invert() {
        float l_det = determinant();
        if (l_det == 0f) throw new NoninvertibleTransformException(this.toString());
    ...
    public Matrix4 setToProjection(float near, float far, float fovy, float aspectRatio) {
        identity();
        float l_fd = (float) (1.0 / Math.tan(Mathf.toRadians(fovy) / 2.0));
    ...
    public Matrix4 setToTranslation(Vector3 vector) {
        return setToTranslation(vector.x, vector.y, vector.z);
    }
    ...
    public Matrix4 setToRotationRad(Vector3 axis, float radians) {
        ...
        return set(quat.setFromAxis(axis, radians));
    }
    ...
    public Matrix4 setToRotationRad(float axisX, float axisY, float axisZ, float radians) {
        ...
        return set(quat.setFromAxis(axisX, axisY, axisZ, radians));
    }
     ...
     * @param y the yaw in degrees
     * @param z the roll in degrees
     * @return This matrix for chaining
     */
    public Matrix4 setFromEulerAngles(float x, float y, float z) {
        quat.setEuler(x, y, z);
        return set(quat);
    }
    ...
    public Quaternion getRotation(Quaternion rotation) {
        return rotation.setFromMatrix(false, this);
    }
	...
	*/
    ...
    public Matrix4 rotate(Quaternion rotation) {
        ...
        tmp[Matrix4.M00] = 1 - 2 * (yy + zz);
        tmp[Matrix4.M01] = 2 * (xy - zw);
        tmp[Matrix4.M02] = 2 * (xz + yw);
        tmp[Matrix4.M03] = 0;
        tmp[Matrix4.M10] = 2 * (xy + zw);
        tmp[Matrix4.M11] = 1 - 2 * (xx + zz);
        tmp[Matrix4.M12] = 2 * (yz - xw);
        tmp[Matrix4.M13] = 0;
        tmp[Matrix4.M20] = 2 * (xz - yw);
        tmp[Matrix4.M21] = 2 * (yz + xw);
        tmp[Matrix4.M22] = 1 - 2 * (xx + yy);
        tmp[Matrix4.M23] = 0;
        tmp[Matrix4.M30] = 0;
        tmp[Matrix4.M31] = 0;
        tmp[Matrix4.M32] = 0;
