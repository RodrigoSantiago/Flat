 ...
 * }</pre>
 */
public class Affine extends AbstractTransform {
    ...
    public Affine(float scale, float angle, float tx, float ty) {
        this(scale, scale, angle, tx, ty);
    }

    /** Creates an affine transform from the supplied scale, rotation and translation. */
    public Affine(float scaleX, float scaleY, float angle, float tx, float ty) {
    ...
    public Affine(float m00, float m01, float m10, float m11, float tx, float ty) {
    ...
    @Override // from Transform
    public Affine setUniformScale (float scale) {
        return (Affine)setScale(scale, scale);
    }

    @Override // from Transform
    public Affine setScaleX (float scaleX) {
    ...
    @Override // from Transform
    public Affine setScaleY (float scaleY) {
        // normalize the scale to 1, then re-apply
    ...
    @Override // from Transform
    public Affine setRotation (float angle) {
    ...
    @Override // from Transform
    public Affine setTranslation (float tx, float ty) {
        this.tx = tx;
        this.ty = ty;
        return this;
    }
    ...
    @Override // from Transform
    public Affine setTx (float tx) {
        this.tx = tx;
        return this;
    }

    @Override // from Transform
    public Affine setTy (float ty) {
        this.ty = ty;
        return this;
    }

    @Override // from Transform
    public Affine setTransform (float m00, float m01, float m10, float m11,
                                float tx, float ty) {
        this.m00 = m00;
        this.m01 = m01;
        this.m10 = m10;
    ...
    @Override // from Transform
    public Affine uniformScale (float scale) {
    ...
    @Override // from Transform
    public Affine scaleX (float scaleX) {
        return Transforms.multiply(this, scaleX, 0, 0, 1, 0, 0, this);
    }

    @Override // from Transform
    public Affine scaleY (float scaleY) {
    ...
    public Transform concatenate (Transform other) {
        ...
        if (other instanceof Affine) {
            return Transforms.multiply(this, (Affine)other, new Affine());
        } else {
            Affine oaff = new Affine(other);
    ...
    public Transform preConcatenate (Transform other) {
        ...
        if (other instanceof Affine) {
            return Transforms.multiply((Affine)other, this, new Affine());
        } else {
            Affine oaff = new Affine(other);
    ...
    public Transform lerp (Transform other, float t) {
        ...
        Affine ot = (other instanceof Affine) ?
            (Affine)other : new Affine(other);
        return new Affine(
    ...
    @Override // from Transform
    public Affine copy () {
        return new Affine(m00, m01, m10, m11, tx, ty);
    }
    ...
    protected Affine(Transform other) {
    ...
    public float getPointX (IPoint p) {
        return m00* p.x() + m10*p.y() + tx;
    }

    public float getPointY (IPoint p) {
        return m01*p.x() + m11*p.y() + ty;
    }
