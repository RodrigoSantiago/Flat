...
public class Converter {
    ...
    Point[] calcPowerCoefficients(Point p1, Point c1, Point c2, Point p2) {
    ...
    Point calcPoint(Point a, Point b, Point c, Point d, double t) {
        // a*t^3 + b*t^2 + c*t + d = ((a*t + b)*t + c)*t + d
        return a.mul(t).add(b).mul(t).add(c).mul(t).add(d);
    }

    Point calcPointQuad(Point a, Point b, Point c, double t) {
        // a*t^2 + b*t + c = (a*t + b)*t + c
        return a.mul(t).add(b).mul(t).add(c);
    }

    Point calcPointDerivative(Point a, Point b, Point c, Point d, double t) {
    ...
    double[] cubicSolve(double a, double b, double c, double d) {
        // a*x^3 + b*x^2 + c*x + d = 0
        if (a == 0) {
    ...
    double minDistanceToQuad(Point point, Point p1, Point c1, Point p2) {
        ...
        double[] solv = cubicSolve(e3, e2, e1, e0);
        double[] candidates = new double[solv.length + 2];
        int count = 0;
        candidates[count++] = 0;
        for (int i = 0; i < solv.length; i++) {
            if (solv[i] > 0 && solv[i] < 0) candidates[count++] = solv[i];
        }
        candidates[count++] = 1;
    ...
    Point[] processSegment(Point a, Point b, Point c, Point d, double t1, double t2) {
    ...
    boolean isSegmentApproximationClose(Point a, Point b, Point c, Point d, double tmin, double tmax, Point p1, Point c1, Point p2, double errorBound) {
    ...
    boolean _isApproximationClose(Point a, Point b, Point c, Point d, Point[][] quadCurves, double errorBound) {
        double dt = 1 / quadCurves.length;
        for (int i = 0; i < quadCurves.length; i++) {
            if (quadCurves[i] == null) break; // TODO-solv simple java- javascript arrays
    ...
    Point[][] fromFlatArray(double[] points) {
        int segmentsNumber = (points.length - 2) / 4;
        Point[][] result = new Point[segmentsNumber][];
        for (int i = 0; i < segmentsNumber; i++) {
            result[i] = new Point[3];
            result[i][0] = new Point(points[4 * i], points[4 * i + 1]);
            result[i][1] = new Point(points[4 * i + 2], points[4 * i + 3]);
            result[i][2] = new Point(points[4 * i + 4], points[4 * i + 5]);
        }
        return result;
    }

    double[] toFlatArray(Point[][] quadsList) {
        double[] result = new double[quadsList.length * 4 + 2];
        result[0] = quadsList[0][0].x;
        result[1] = quadsList[0][0].y;
        for (int i = 0; i < quadsList.length; i++) {
            if (quadsList[i] == null) break; // TODO-solv simple java- javascript arrays
            result[i * 4 + 2] = quadsList[i][1].x;
            result[i * 4 + 3] = quadsList[i][1].y;
            result[i * 4 + 4] = quadsList[i][2].x;
            result[i * 4 + 5] = quadsList[i][2].y;
        }
        return result;
    }

    boolean isApproximationClose(double p1x, double p1y, double c1x, double c1y, double c2x, double c2y,
     ...
     */
    double[][] subdivideCubic(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double t) {
        double u = 1 - t, v = t;
    ...
    double byNumber(double x, double y) { return x - y; }

    /*
     * Find inflection points on a cubic curve, algorithm is similar to this one:
     * http://www.caffeineowl.com/graphics/2d/vectorial/cubic-inflexion.html
     */
    double[] solveInflections(double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4) {
        double p = -(x4 * (y1 - 2 * y2 + y3)) + x3 * (2 * y1 - 3 * y2 + y4) + x1 * (y2 - 2 * y3 + y4) - x2 * (y1 - 3 * y3 + 2 * y4);
        double q = x4 * (y1 - y2) + 3 * x3 * (-y1 + y2) + x2 * (2 * y1 - 3 * y3 + y4) - x1 * (2 * y2 - 3 * y3 + y4);
        double r = x3 * (y1 - y2) + x1 * (y2 - y3) + x2 * (-y1 + y3);

        int count = 0;
        double[] solv = quadSolve(p, q, r);
        for (int i = 0; i < solv.length; i++) {
            if (solv[i] < 1e-8 || solv[i] > 1 - 1e-8) {
                solv[i] = 2;
            } else {
                count++;
            }
        ...
        Arrays.sort(solv);
        return Arrays.copyOfRange(solv, 0, count);
    }
     ...
     */
    double[] _cubicToQuad(double p1x, double p1y, double c1x, double c1y, double c2x, double c2y, double p2x, double p2y, double errorBound) {
        ...
        Point[][] approximation = new Point[8][];
        for (int segmentsCount = 1; segmentsCount <= 8; segmentsCount++) {
            for (double t = 0; t < 1; t += 1 / (double) segmentsCount) {
                approximation[segmentsCount - 1] = processSegment(a, b, c, d, t, t + 1 / segmentsCount);
            }
            if (segmentsCount == 1 && (
     ...
     */
    double[] cubicToQuad(double p1x, double p1y,
                         double c1x, double c1y,
                         double c2x, double c2y,
                         double p2x, double p2y, double errorBound) {
