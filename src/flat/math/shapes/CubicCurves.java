 ...
 */
public final class CubicCurves {
    ...
    public static float flatness (float x1, float y1, float ctrlx1, float ctrly1,
                                     float ctrlx2, float ctrly2, float x2, float y2) {
        return Mathf.sqrt(flatnessSq(x1, y1, ctrlx1, ctrly1, ctrlx2, ctrly2, x2, y2));
    }
    ...
    public static void subdivide (CubicCurve src, CubicCurve left, CubicCurve right) {
        ...
        cy = (ay + by) / 2f;
        if (left != null) {
            left.set(x1, y1, cx1, cy1, ax, ay, cx, cy);
        }
        if (right != null) {
            right.set(cx, cy, bx, by, cx2, cy2, x2, y2);
        }
    }
