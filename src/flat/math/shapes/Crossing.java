...
public final class Crossing {
    ...
    public static int solveCubic (float[] eqn, float[] res) {
        ...
        if (R2 < Q3) {
            float t = Mathf.acos(R / Mathf.sqrt(Q3)) / 3f;
            float p = 2f * Mathf.PI / 3f;
            float m = -2f * Mathf.sqrt(Q);
            res[rc++] = m * Mathf.cos(t) + n;
            res[rc++] = m * Mathf.cos(t + p) + n;
            res[rc++] = m * Mathf.cos(t - p) + n;
        } else {
            // Debug.println("R2 >= Q3 (" + R2 + "/" + Q3 + ")");
            float A = Mathf.pow(Math.abs(R) + Mathf.sqrt(R2 - Q3), 1f / 3f);
            if (R > 0f) {
                A = -A;
            }
            // if (A == 0f) {
    ...
    public static int solveCubic (double[] eqn, double[] res) {
        ...
        if (R2 < Q3) {
            ...
            // Debug.println("R2 >= Q3 (" + R2 + "/" + Q3 + ")");
            double A = Math.pow(Math.abs(R) + Math.sqrt(R2 - Q3), 1d / 3d);
            if (R > 0f) {
                A = -A;
            }
            // if (A == 0f) {
