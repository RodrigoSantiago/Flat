...
/*Code adapted by Hanns Holger Rutz based originally on*/
...
public class ShapeInterpolator {
    ...
    private static class Geometry {
        ...
        public Geometry(Shape s) {
            ...
            final PathIterator pi = s.pathIterator(null);
            ...
            float newX, newY;
            final Vector<Vector2> savedPathEndPoints = new Vector<Vector2>();
            numCoordinates = 2;
            while (!pi.isDone()) {
                switch (pi.currentSegment(coordinates)) {
                    case PathIterator.SEG_MOVETO:
                        ...
                        if (curX != newX || curY != newY) {
                            savedPathEndPoints.add(new Vector2(movX, movY));
            ...
            for (int i = savedPathEndPoints.size()-1; i >= 0; i--) {
                final Vector2 p = savedPathEndPoints.get(i);
                newX = p.x;
                newY = p.y;
                if (curX != newX || curY != newY) {
                    appendLineTo(curX, curY, newX, newY);
    ...
    private static class MorphedShape implements Shape {
        ...
        public boolean contains(float x, float y) {
            return false;//Path.contains(pathIterator(null), x, y);
        }
        ...
        public boolean intersects(float x, float y, float w, float h) {
            return false;//Path.intersects(pathIterator(null), x, y, w, h);
        }
        ...
        public boolean contains(float x, float y, float width, float height) {
            return false;//Path.contains(pathIterator(null), x, y, width, height);
        }
        ...
        @Override
        public MorphedShape clone() {
            return new MorphedShape(geom0, geom1, t, unionBounds);
        }
    }
